      ...
      <div className="max-w-5xl w-full">
        ...
        {!isTeamMode && (
          <div className="text-center mb-6 md:mb-8 animate-fade-in">
            <Trophy className="w-16 h-16 md:w-20 md:h-20 text-yellow-400 mx-auto mb-4 animate-bounce-slow" />
            <h1 className="text-3xl md:text-6xl font-bold mb-4 bg-gradient-to-r from-yellow-400 via-orange-400 to-red-400 text-transparent bg-clip-text">
        ...
        {isTeamMode && (
          <div className="text-center mb-6 md:mb-8 animate-fade-in">
            <Trophy className="w-16 h-16 md:w-20 md:h-20 text-yellow-400 mx-auto mb-4 animate-bounce-slow" />
            <h1 className={`text-3xl md:text-6xl font-bold mb-4 bg-gradient-to-r ${
        ...
        {/* Rankings */}
        <div className="glass p-4 md:p-8 rounded-2xl mb-6">
          <h2 className="text-2xl md:text-3xl font-bold mb-4 md:mb-6 text-center">Final Rankings</h2>
          ...
          {!isTeamMode && (
            <div className="space-y-3">
              ...
              <div
                key={player.playerId}
                className={`flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-4 p-3 md:p-4 rounded-xl transition-all ${
                  player.playerId === playerId
                    ? 'bg-blue-500/30 border-2 border-blue-400 md:scale-105'
                    : 'glass-dark'
                }`}
              >
                {/* Rank */}
                <div className={`w-12 h-12 md:w-16 md:h-16 rounded-full bg-gradient-to-br ${getMedalColor(player.rank)} flex items-center justify-center text-xl md:text-2xl font-bold flex-shrink-0`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-lg md:text-xl font-bold truncate">{player.username}</span>
                    {player.playerId === playerId && (
                      <span className="text-xs px-2 py-1 bg-blue-500/30 rounded">You</span>
                    )}
                  </div>
                  <div className="flex flex-wrap items-center gap-2 md:gap-4 text-xs md:text-sm text-blue-200">
                ...
                <div className="grid grid-cols-3 gap-2 md:gap-4 text-center w-full md:w-auto">
                  <div className="glass-dark px-2 md:px-4 py-2 rounded-lg">
                    <Clock className="w-3 h-3 md:w-4 md:h-4 mx-auto mb-1 text-blue-400" />
                    <div className="text-xs md:text-sm font-mono">
                      {player.hasFinished ? formatTime(player.completionTime) : 'N/A'}
                    </div>
                  </div>
                  <div className="glass-dark px-2 md:px-4 py-2 rounded-lg">
                    <Target className="w-3 h-3 md:w-4 md:h-4 mx-auto mb-1 text-green-400" />
                    <div className="text-xs md:text-sm font-mono">
                      {player.distanceToEnd === 0 ? '0' : player.distanceToEnd}
                    </div>
                  </div>
                  <div className="glass-dark px-2 md:px-4 py-2 rounded-lg">
                    <Footprints className="w-3 h-3 md:w-4 md:h-4 mx-auto mb-1 text-purple-400" />
                    <div className="text-xs md:text-sm font-mono">{player.moves}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        {!isTeamMode && (
          <div className="glass p-4 md:p-6 rounded-xl mb-6">
            <h3 className="text-lg md:text-xl font-bold mb-4 text-center">Game Statistics</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
              <div className="text-center">
                <div className="text-2xl md:text-3xl font-bold text-blue-400">
                  {gameResults.statistics.totalPlayers}
                </div>
                <div className="text-xs md:text-sm text-blue-200">Total Players</div>
              </div>
              <div className="text-center">
                <div className="text-2xl md:text-3xl font-bold text-green-400">
                  {gameResults.statistics.finishers}
                </div>
                <div className="text-xs md:text-sm text-blue-200">Finished</div>
              </div>
              <div className="text-center">
                <div className="text-2xl md:text-3xl font-bold text-purple-400">
                  {formatTime(gameResults.statistics.averageTime)}
                </div>
                <div className="text-xs md:text-sm text-blue-200">Avg. Time</div>
              </div>
              <div className="text-center">
                <div className="text-2xl md:text-3xl font-bold text-yellow-400">
                  {gameResults.statistics.averageMoves}
                </div>
                <div className="text-xs md:text-sm text-blue-200">Avg. Moves</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col md:flex-row gap-3 md:gap-4">
          <button
            onClick={leaveRoom}
            className="flex-1 px-4 md:px-6 py-3 md:py-4 rounded-xl glass hover:bg-white/20 transition-all font-semibold flex items-center justify-center gap-2"
          >
          ...
          {isHost ? (
            <button
              onClick={restartRoom}
              className="flex-1 px-4 md:px-6 py-3 md:py-4 rounded-xl bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 transition-all font-semibold flex items-center justify-center gap-2"
            >
              <RotateCcw className="w-5 h-5" />
              <span className="hidden md:inline">Play Again (Return to Lobby)</span>
              <span className="md:hidden">Play Again</span>
            </button>
          ) : (
            <div className="flex-1 px-4 md:px-6 py-3 md:py-4 rounded-xl glass text-center">
              <p className="text-sm md:text-base text-blue-200">Waiting for host to restart...</p>
            </div>
          )}
        </div>
      </div>
